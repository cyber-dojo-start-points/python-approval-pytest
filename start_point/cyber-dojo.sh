...
coverage run \
  ...
  --approvaltests-use-reporter='PythonNative' > ${HELD}/tests 2> ${HELD}/tests.err &
readonly TESTS_PID=$!

# --------------------------------------------------------------
# Replayed in the order running them one after another would have printed.
# No blank line before the tests here, unlike its siblings, because the
# sequence this replaces had none.
echo MyPy
wait ${MYPY_PID}
